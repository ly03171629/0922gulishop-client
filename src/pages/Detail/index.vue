...
export default {
  ...
  methods: {
    ...
    changeChecked(spuSaleAttrValueList, spuSaleAttrValue) {
      ...
      // 排它第一步：  让所有的成员全部都变为同一种状态
      spuSaleAttrValueList.forEach((item) => (item.isChecked = "0"));
      // 排它第二步：  让当前点击的这一个成员变为另一种状态
      spuSaleAttrValue.isChecked = "1";
    },
    //点击添加购物车逻辑
    async addShopCart(){
      //第一步：先发请求添加购物车
      //dispatch的英文意思：触发分发的意思，本质就是在调用相关的action函数
      //现在我想要在组件这里知道请求是成功的还是失败的
      // 我们只能通过dispatch的返回值，来判断请求是成功的还是失败的
      // 目前我们下面这行代码，结果一定是promise
  ...
  computed: {
    ...
    imgList() {
      // 如果直接这样写，很可能是undefined
      // 当数据请求回来之前，这个值就是undefined
      // 我们也必须在这里或一个[]
      // 如果skuInfo.skuImageList是一个undefined，那么我们应该给一个[]
      // 确保这个imgList，最后不是undefined
